export const COMMON_TRANSLATIONS_RU = {
    "HOME":
        "ГЛАВНАЯ",

    "SERVICES":
        "УСЛУГИ",

    "NEWS":
        "НОВОСТИ",

    "LANGUAGE":
        "ЯЗЫК",

    "Dragon Chat":
        "Dragon Chat",

    "Welcome.":
        "Добро пожаловать.",

    "Dragon is thinking...":
        "Dragon думает...",

    "What Can We Do for You?":
        "Чем мы можем вам помочь?",

    "We develop modern AI solutions for businesses — from research and concept design to production-ready intelligent systems.":
        "Мы разрабатываем современные ИИ-решения для бизнеса: от исследований и проектирования концепции до готовых к запуску интеллектуальных систем.",

    "AI Strategy & Consulting":
        "Стратегия и консалтинг в области ИИ",

    "We help companies understand where AI creates real value, define priorities and turn ambitious ideas into a clear implementation path.":
        "Мы помогаем компаниям понять, где ИИ создает реальную ценность, определить приоритеты и превратить амбициозные идеи в понятный план внедрения.",

    "AI Consulting":
        "Консалтинг по ИИ",

    "AI Roadmaps":
        "Дорожные карты ИИ",

    "AI MVP Development":
        "Разработка MVP на базе ИИ",

    "Business Process Automation":
        "Автоматизация бизнес-процессов",

    "AI Development":
        "Разработка ИИ",

    "We design, build and train intelligent systems tailored to your product, data and business processes.":
        "Мы проектируем, создаем и обучаем интеллектуальные системы под ваш продукт, данные и бизнес-процессы.",

    "Custom AI Models":
        "Кастомные ИИ-модели",

    "Machine Learning Models":
        "Модели машинного обучения",

    "Fine-Tuning Existing Models":
        "Дообучение существующих моделей",

    "Training Models on Custom Datasets":
        "Обучение моделей на данных клиента",

    "Training AI Models from Scratch":
        "Обучение моделей ИИ с нуля",

    "Custom Neural Network Architectures":
        "Индивидуальные архитектуры нейросетей",

    "LLM & Generative AI":
        "LLM и генеративный ИИ",

    "We integrate language models, knowledge bases and generative tools into practical business workflows.":
        "Мы интегрируем языковые модели, базы знаний и генеративные инструменты в реальные бизнес-процессы.",

    "RAG Systems":
        "RAG-системы",

    "LLM Integration":
        "Интеграция LLM",

    "Enterprise Knowledge Bases":
        "Корпоративные базы знаний",

    "Generative AI":
        "Генеративный ИИ",

    "AI Agents & Multi-Agent Systems":
        "ИИ-агенты и мультиагентные системы",

    "Computer Vision":
        "Компьютерное зрение",

    "We build visual intelligence for image analysis, object detection, monitoring, classification and quality control.":
        "Мы создаем визуальный интеллект для анализа изображений, обнаружения объектов, мониторинга, классификации и контроля качества.",

    "Data Analytics":
        "Аналитика данных",

    "Recommendation Systems":
        "Рекомендательные системы",

    "Anomaly Detection":
        "Обнаружение аномалий",

    "AI Evaluation & Benchmarking":
        "Оценка и бенчмаркинг ИИ",

    "We define quality standards, evaluate model behavior and create reliable benchmarks for technical decisions.":
        "Мы задаем стандарты качества, оцениваем поведение моделей и создаем надежные бенчмарки для технических решений.",

    "Benchmark Development":
        "Разработка бенчмарков",

    "AI Evaluation":
        "Оценка ИИ",

    "ML Metrics":
        "Метрики машинного обучения",

    "Predictive Analytics":
        "Предиктивная аналитика",

    "Research & Enterprise AI":
        "Исследовательский и корпоративный ИИ",

    "We prototype advanced architectures, explore experimental ideas and prepare scalable AI systems for long-term enterprise use.":
        "Мы прототипируем продвинутые архитектуры, исследуем экспериментальные идеи и готовим масштабируемые ИИ-системы для долгосрочного корпоративного использования.",

    "Research & Development (R&D)":
        "Исследования и разработка (R&D)",

    "Enterprise AI Integration":
        "Интеграция корпоративного ИИ",

    "Custom AI Development":
        "Индивидуальная разработка ИИ",

    "Requirements Analysis":
        "Анализ требований",

    "We clarify the business goal, technical constraints, data sources and expected result.":
        "Мы уточняем бизнес-цель, технические ограничения, источники данных и ожидаемый результат.",

    "Solution Architecture":
        "Архитектура решения",

    "We define the AI system structure, integrations, data flow and development roadmap.":
        "Мы определяем структуру ИИ-системы, интеграции, поток данных и дорожную карту разработки.",

    "We build the core intelligence, interfaces and automation logic required for the product.":
        "Мы создаем ядро интеллектуальной системы, интерфейсы и логику автоматизации, необходимые продукту.",

    "Model Training":
        "Обучение модели",

    "We prepare data, train or fine-tune models and align them with the target use case.":
        "Мы готовим данные, обучаем или дообучаем модели и адаптируем их под целевой сценарий.",

    "Evaluation & Benchmarking":
        "Оценка и бенчмаркинг",

    "We test model quality with benchmarks, metrics and practical performance checks.":
        "Мы проверяем качество модели с помощью бенчмарков, метрик и практических тестов производительности.",

    "Deployment":
        "Развертывание",

    "We prepare the solution for real users, production environments and business workflows.":
        "Мы готовим решение к работе с реальными пользователями, production-средой и бизнес-процессами.",

    "Support & Future Development":
        "Поддержка и дальнейшее развитие",

    "We continue improving the system as your data, users and goals evolve.":
        "Мы продолжаем улучшать систему по мере развития ваших данных, пользователей и целей.",

    "Why Golden Dragon AI Studio":
        "Почему Golden Dragon AI Studio",

    "Our Help for Your Business":
        "Как мы помогаем вашему бизнесу",

    "We help companies implement artificial intelligence, automate business processes and build intelligent products that increase efficiency, reduce costs and create new growth opportunities.":
        "Мы помогаем компаниям внедрять искусственный интеллект, автоматизировать бизнес-процессы и создавать интеллектуальные продукты, которые повышают эффективность, снижают затраты и открывают новые возможности роста.",

    "We help companies implement artificial intelligence, automate business processes and build intelligent products that increase efficiency, reduce costs and create new business opportunities.":
        "Мы помогаем компаниям внедрять искусственный интеллект, автоматизировать бизнес-процессы и создавать интеллектуальные продукты, которые повышают эффективность, снижают затраты и открывают новые бизнес-возможности.",

    "Choose the direction that best matches your business needs.":
        "Выберите направление, которое лучше всего соответствует задачам вашего бизнеса.",

    "Tailor-made AI solutions":
        "ИИ-решения под конкретную задачу",

    "Full-cycle AI development":
        "Полный цикл разработки ИИ",

    "Training on customer datasets":
        "Обучение на данных клиента",

    "Benchmarking & evaluation":
        "Бенчмаркинг и оценка",

    "Modern AI technologies":
        "Современные ИИ-технологии",

    "Scalable architectures":
        "Масштабируемые архитектуры",

    "Long-term technical support":
        "Долгосрочная техническая поддержка",

    "Research & Innovation Showcase":
        "Витрина исследований и инноваций",

    "Building the Future of Intelligent AI Products":
        "Создаем будущее интеллектуальных ИИ-продуктов",

    "Explore the proprietary AI technologies, intelligent systems and next-generation products currently being developed by Golden Dragon AI Studio.":
        "Изучите собственные ИИ-технологии, интеллектуальные системы и продукты нового поколения, которые сейчас разрабатывает Golden Dragon AI Studio.",

    "Golden Dragon AI Studio develops proprietary artificial intelligence solutions that combine advanced research, modern AI technologies and real-world engineering.":
        "Golden Dragon AI Studio разрабатывает собственные решения искусственного интеллекта, объединяющие продвинутые исследования, современные ИИ-технологии и практическую инженерию.",

    "This section presents our original AI products, intelligent assistants and industry-focused technologies currently under active development. Each solution demonstrates our engineering approach, system architecture and long-term product vision.":
        "В этом разделе представлены наши оригинальные ИИ-продукты, интеллектуальные ассистенты и отраслевые технологии, которые находятся в активной разработке. Каждое решение показывает наш инженерный подход, системную архитектуру и долгосрочное видение продукта.",

    "Our technologies are designed both as commercial AI products and as flexible technology platforms that can be adapted to solve complex business challenges across multiple industries.":
        "Наши технологии создаются и как коммерческие ИИ-продукты, и как гибкие технологические платформы, которые можно адаптировать для решения сложных бизнес-задач в разных отраслях.",

    "For Clients":
        "Для клиентов",

    "Our AI solutions demonstrate the technologies and engineering capabilities that can be customized for your organization.":
        "Наши ИИ-решения демонстрируют технологии и инженерные возможности, которые можно адаптировать под вашу организацию.",

    "Depending on your business objectives, we can adapt existing platforms or develop completely new AI systems using the same architecture, including enterprise AI assistants, Computer Vision, multimodal AI, Retrieval-Augmented Generation (RAG), intelligent automation and custom machine learning models.":
        "В зависимости от ваших бизнес-целей мы можем адаптировать существующие платформы или разработать полностью новые ИИ-системы на той же архитектуре, включая корпоративных ИИ-ассистентов, компьютерное зрение, мультимодальный ИИ, RAG, интеллектуальную автоматизацию и индивидуальные модели машинного обучения.",

    "For Investors":
        "Для инвесторов",

    "Several projects presented in this showcase are being developed as long-term technology platforms with significant commercial potential.":
        "Несколько проектов в этой витрине развиваются как долгосрочные технологические платформы с серьезным коммерческим потенциалом.",

    "Included:":
        "Включает:",

    "How We Work":
        "Как мы работаем",

    "Start Your Project":
        "Начните проект",

    "Tell us about your task.":
        "Расскажите нам о вашей задаче.",

    "We will help you define the optimal AI solution, prepare a project concept and choose the most effective implementation path.":
        "Мы поможем определить оптимальное ИИ-решение, подготовить концепцию проекта и выбрать самый эффективный путь внедрения.",

    "Describe your project and Dragon will help you find the best AI solution.":
        "Опишите проект, и Dragon поможет подобрать лучшее ИИ-решение.",

    "Documents":
        "Документы",

    "Service Agreement":
        "Договор на оказание услуг",

    "We welcome discussions with strategic partners and investors interested in supporting the development and global commercialization of next-generation AI products.":
        "Мы открыты к обсуждению сотрудничества со стратегическими партнерами и инвесторами, заинтересованными в развитии и глобальной коммерциализации ИИ-продуктов нового поколения.",

    "Technology Areas":
        "Технологические направления",

    "Available AI Solutions":
        "Доступные ИИ-решения",

    "Intellectual Property":
        "Интеллектуальная собственность",

    "The AI technologies, software architectures, intelligent systems, concepts, algorithms, datasets, product designs and visual materials presented in this showcase represent proprietary developments of Golden Dragon AI Studio.":
        "ИИ-технологии, программные архитектуры, интеллектуальные системы, концепции, алгоритмы, наборы данных, продуктовые дизайны и визуальные материалы, представленные в этой витрине, являются собственными разработками Golden Dragon AI Studio.",

    "These technologies demonstrate our research, engineering capabilities and long-term product vision for commercial AI systems and custom enterprise solutions.":
        "Эти технологии демонстрируют наши исследовательские и инженерные возможности, а также долгосрочное видение коммерческих ИИ-систем и индивидуальных корпоративных решений.",

    "Selected AI Engineering Projects":
        "Избранные инженерные ИИ-проекты",

    "Explore a selection of AI systems, research projects and production platforms developed across computer vision, multimodal AI, enterprise AI, healthcare, optimization, autonomous systems and quantitative finance.":
        "Изучите подборку ИИ-систем, исследовательских проектов и промышленных платформ в области компьютерного зрения, мультимодального ИИ, корпоративного ИИ, здравоохранения, оптимизации, автономных систем и количественных финансов.",

    "Interested in building a similar AI solution?":
        "Хотите создать похожее ИИ-решение?",

    "Our team can help transform your idea into a production-ready AI system tailored to your business needs.":
        "Наша команда поможет превратить вашу идею в готовую к запуску ИИ-систему, адаптированную под задачи вашего бизнеса.",

    "Latest updates, research, product releases and development news from Golden Dragon AI Studio.":
        "Последние обновления, исследования, релизы продуктов и новости разработки от Golden Dragon AI Studio.",

    "Coming Soon":
        "Скоро"
};
